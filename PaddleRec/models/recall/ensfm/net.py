...
class ENSFMLayer(nn.Layer):
    def __init__(self, num_users, num_items, embedding_size):
        ...
        self.user_feature_emb = nn.Embedding(self.num_users, self.embedding_size,
        ...
        self.H_s = paddle.create_parameter([self.embedding_size, 1], default_type,
                                           default_initializer=nn.initializer.Constant(0.01))
        self.all_item_feature_emb = nn.Embedding(self.num_items + 1, self.embedding_size,
                                                 weight_attr=paddle.ParamAttr(
                                                     initializer=paddle.nn.initializer.TruncatedNormal(std=0.01)))
    ...
    def forward(self, input_u, item_attribute, input_ur=None, item_bind_M=None):
        ...
        pos_r = paddle.einsum('ajk,kl->ajl', pos_r, H_i_emb).reshape([input_u.shape[0], -1])
